...
export function runChain(vaultPath: string, rules: Rule[]): ChainTrace {
	...
	for (let index = 0; index < rules.length; index++) {
		...
		let regex: RegExp;
		// INVARIANT: this `try` must stay scoped to the `new RegExp` compile below and
		// must never be widened to also cover the test()/replace() calls further down.
		// A throw between `matched = true` and the `acc` reassignment would leave
		// `matched` set with a stale `acc`, silently suppressing the basename fallback
		// and shipping a wrong title — that failure mode, not merely "what does this
		// catch", is why the try stops here.
		try {
			...
			// decision: architecture/decisions/2026-08-18-OTR-0010-runchain-carries-a-compile-only-crash-barrier.md
